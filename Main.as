package  {
	...
	public class Main extends MovieClip
	{
		...
		var globalTop10List:MovieClip;
		
		var friendsTop10List_txt:TextField;
		var friendsTop10List:MovieClip;
		
		//Menu buttons
		var startButton:StartButton;
		var inviteFriendsButton:InviteFriendsButton;
		var topListsButton:TopListsButton;
		
		//Main Menu
		var mainMenuPanel:MainMenuPanel;
		
		//Screen indicator 0-main menu , 1-top list screen
		...
		{   
            ...
            swfStage.scaleMode = StageScaleMode.NO_SCALE;  //So that the game doesn't scale when stretched
			...
			init_main_menu();             //initialize menu		
		...
		{			
			...
			//Main menu buttons
			inviteFriendsButton = new InviteFriendsButton(this);
            startButton = new StartButton(this);
			topListsButton = new TopListsButton(this);
			
			//Top lists			
			friendsTop10List = new FriendsTop10List(this);
			friendsTop10List_txt = friendsTop10List.friends_top10_txt;
			
			globalTop10List = new GlobalTop10List(this);
			globalTop10List_txt = globalTop10List.global_top10_txt;
			
			//listeners
			startButton.addEventListener(MouseEvent.MOUSE_DOWN, startNewGame);	 
			topListsButton.addEventListener(MouseEvent.MOUSE_DOWN, goToTopListsScreen);	
			inviteFriendsButton.addEventListener(MouseEvent.MOUSE_DOWN, callInviteFriendsPopup);	
			
			arrangeElementsForMainMenuPage();	
		...
		function init_pause_system()
		{
			...
			setChildIndex(pauseOverlay, numChildren-1);
			pauseOverlay.visible=false;			
		}	
		
		//Reset all game elements so everything is ready for a new game
		function resetGame()
		{						
			//Delete all boats and their trajectories
		    for(var i:Number=0; i<all_boats.length; i++)
			{
			    var bo:BoatController = all_boats[i];					
				bo.erase_all_trajectory_line_graphics();
				//trace("all boats length je"+all_boats.length);
				removeChild(bo);
			}		
			
			//Initiate boats array to new array
			init_boats();
			
		}
		
		
		function startNewGame(event:MouseEvent):void
		{
			if(screenIndicator==0)
			deleteAllMainMenuElements();
			
			if(screenIndicator==1)
			deleteAllTop10ScreenElements();		
		...
		{   
			...
			if(navigated_boat!=null)
			{
			  ...
			  //navigated_boat.set_one_before_last_trajectory_point(mouseX,mouseY); 
			  navigated_boat.set_one_before_last_trajectory_point(null, null);  //POSSIBLE BOAT TRAJECTORY BUG
		...
		//Deletes and removes from displayContainer menu elements (buttons, top lists...)
		function deleteAllMainMenuElements()
		{
			removeChild(mainMenuPanel);
			removeChild(startButton);
			removeChild(inviteFriendsButton);
			removeChild(topListsButton);
		}
		
		//Deletes and removes from displayContainer menu elements (buttons, top lists...)
		function deleteAllTop10ScreenElements()
		{
			removeChild(mainMenuPanel);
			removeChild(startButton);
			removeChild(inviteFriendsButton);									
				
			//removeChild(friendsTop10List_txt);
			//removeChild(globalTop10List_txt);
			
			removeChild(globalTop10List);
			removeChild(friendsTop10List);
			
		}
		
		//Buttons on top list page need to be rearranged - put them below the lists
		public function arrangeElementsForToplistScreen()
		{
			screenIndicator=1;
			
			addChild(mainMenuPanel);
			
		    addChild(globalTop10List);
		    addChild(friendsTop10List);
			
			//addChild(globalTop10List_txt);
			//addChild(friendsTop10List_txt);
			
		    addChild(startButton);
		    addChild(inviteFriendsButton);			
			
			
		    globalTop10List_txt.x=12;
            globalTop10List_txt.y=14;
            globalTop10List_txt.width=320;
            globalTop10List_txt.height=520;
 
            friendsTop10List_txt.x=344;
            friendsTop10List_txt.y=14;
            friendsTop10List_txt.width=320;
            friendsTop10List_txt.height=520;

            startButton.x=175;
            startButton.y=500;
			
            inviteFriendsButton.x=420;
            inviteFriendsButton.y=500;						 
						
			
			//Bring buttons to front
			var mp:DisplayObjectContainer = inviteFriendsButton.parent;
            mp.setChildIndex(inviteFriendsButton, mp.numChildren-1);
			mp.setChildIndex(startButton, mp.numChildren-1);
		    
		    
		}
		
		//Arrange elements for display on main menu page
		function arrangeElementsForMainMenuPage()
		{
			screenIndicator=0;
			...
			topListsButton.x=352;
            topListsButton.y=232;		

            inviteFriendsButton.x=242;
            inviteFriendsButton.y=350;         	
            ...
            mp.setChildIndex(inviteFriendsButton, mp.numChildren-1);
			mp.setChildIndex(startButton, mp.numChildren-1);
			mp.setChildIndex(topListsButton, mp.numChildren-1);
		    
		    
		}
		
		function goToTopListsScreen(event:MouseEvent)
		{
			deleteAllMainMenuElements();
		    gotoAndPlay(5);
		...
		function callInviteFriendsPopup(event:MouseEvent)
		{
			if(ExternalInterface.available) trace("AVAILABLE") else trace("UNAVAILABLE");
